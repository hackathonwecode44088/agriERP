...
const Sales = () => {
  ...
  const meta = CATEGORY_META[category] || { label: category, buyer: "farmers", buyerLabel: "Farmer" };
  const partySource = meta.buyer;
  const productSource = `products?category=${category}`;

  return (
    <CrudPage
      key={category}
      testid={`sales-${category}`}
      title={`Sell ${meta.label}`}
      subtitle={`Invoice ${meta.label.toLowerCase()} sales to ${meta.buyerLabel.toLowerCase()}s. Farmer sales post to the farmer ledger automatically.`}
      endpoint="sales"
      query={{ category }}
      ...
      fields={[
        ...
        { name: "party_id", label: meta.buyerLabel, type: "select", optionsFrom: partySource },
      ...
      columns={[
        ...
        { key: "date", label: "Date" },
        { key: "party_id", label: meta.buyerLabel, render: (r, lk) => nameOf(lk[partySource], r.party_id) },
